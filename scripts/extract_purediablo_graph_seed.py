...
def classify_url(url: str) -> tuple[str, str]:
    ...
    mapping = {
        "Horadric_Cube": "RecipePage",
        "Runewords": "RunewordIndex",
        "Unique_Items": "UniqueItemIndex",
        "Amazon": "Class",
        "Paladin": "Class",
        "Necromancer": "Class",
        "Necromancer_Skills": "SkillPage",
        "Necromancer_Poison_and_Bone": "SkillPage",
        "Necromancer_Items": "ItemGuidePage",
        "Diablo_II_Manual": "ReferenceManual",
        "V1.10_FAQ": "ReferenceFAQ",
    }
    return mapping.get(slug, "WikiPage"), slug


def main() -> int:
    parser = argparse.ArgumentParser(description="Extract graph seed from PureDiablo high-value corpus")
    parser.add_argument("--documents", default="docs/tier0/purediablo-high-value/normalized/documents.jsonl")
    parser.add_argument("--output-root", default="docs/tier0/purediablo-high-value/derived")
    ...
    for doc in docs:
        source_node_id = make_id("source", "purediablo-d2wiki")
        upsert(source_node_id, node_type="Source", key="purediablo-d2wiki", name="PureDiablo D2Wiki")
        doc_node_id = make_id("pd-doc", doc["doc_id"])
        upsert(
            doc_node_id,
        ...
        entity_node_id = make_id("pd-entity", f"{entity_type}|{entity_key}")
        upsert(
            entity_node_id,
    ...
    (output_root / "graph-seed-report.md").write_text(
        "# PureDiablo High-Value Graph Seed Report\n\n"
